...
class WaterMqttConverterMain():
    ...
    def start(self):
        ...
        if self.robotid == "":
            ...
            return False
        rospy.Subscriber("/mqtt_bridge/mqtt_topic_go_down", Mqtt_test, self.mqtt_topic_go_down_callback)
        rospy.Subscriber("/mqtt_bridge/connect_status_test", String, self.mqtt_bridge_connect_status_callback)
        self.pub_mqtt_go_up = rospy.Publisher('/mqtt_bridge/mqtt_topic_go_up', Mqtt_test, queue_size=10, latch=True)
        return True

    def sub_twist(self):
        rospy.loginfo("sub twist")
        m = Mqtt_test()
        #m.name = 'cmd_vel'
        m.type = 'sub'
    ...
    def sub_download_map_notice(self):
        rospy.loginfo("sub download map notice")
        m = Mqtt_test()
       # m.name = 'robot_map_info'
        m.type = 'sub'
        m.robotid = self.robotid
        self.pub_mqtt_go_up.publish(m)
    ...
    def mqtt_topic_go_down_callback(self, data):
        ...
        '''
        if data.type == 'topic':
            if data.name == 'cmd_vel':
                t = json_message_converter.convert_json_to_ros_message('geometry_msgs/Twist', data.data)
                self.twist_pub.publish(t)
            elif data.name == 'robot_map_info':
                self.handle_map_notice(data.data)
        '''
        if data.type == 'move':
            ...
            urllib2.urlopen("http://localhost:8899/api/move?marker="+mqtt_data["position"])
            return_data = Mqtt_test()
            return_data.type = "callback"
            data_dict = {"cb":mqtt_data['cb'], "status":"20001", "msg":"success"}
